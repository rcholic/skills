...
推荐语（recommendationText）：140字内，站在创业者角度。要求具体、可操作，避免「提升效率」「值得关注」等空泛表述。
- 场景：谁在什么阶段/情境下会用到（如：早期创始人、做增长时、招技术合伙人时）。
- 问题：具体痛点，避免泛词，可带一句可感知的后果。
- 解决方案：这条内容/产品/方法具体做了什么（一句话能复述）。
- 价值：对创业者可操作的收获——可借鉴的一招、可复用的思路或可避的坑。
若内容与创业关联较弱，可只写场景与价值，不必强行凑齐四段。`;
    ...
    const userPrompt = `请为以下分组生成 groupSummary（100字内，概括本组对应的创业者需求/关注点）和每条条目的 recommendationText（140字内，创业者视角；格式：场景、问题、解决方案、价值；关联弱时可只写场景与价值）。\n只输出一个 JSON，格式：{"groups":[{"groupSummary":"...","items":[{"id":"...","recommendationText":"..."}]}]}\n\n分组与条目：\n${JSON.stringify(payload, null, 2)}`;
