...
- User wants Marineyang to appear in a specific outfit, location, or situation
...
**Example**: "wearing a designer dress" →
```
make a pic of this person, but wearing a designer dress. the person is taking a mirror selfie
```
...
**Example**: "a fancy rooftop restaurant" →
```
a close-up selfie taken by herself at a fancy rooftop restaurant, direct eye contact with the camera, looking straight into the lens, eyes centered and clearly visible, not a mirror selfie, phone held at arm's length, face fully visible
```
...
```bash
REFERENCE_IMAGE="https://cdn.jsdelivr.net/gh/swancho/clawaifu@main/assets/marineyang.png"
...
if [ -z "$USER_CONTEXT" ] || [ -z "$CHANNEL" ]; then
  ...
  echo "Example: $0 'wearing a designer dress' '#general' mirror"
  echo "Example: $0 'a fancy cafe' '#general' direct"
  exit 1
fi
...
const REFERENCE_IMAGE = "https://cdn.jsdelivr.net/gh/swancho/clawaifu@main/assets/marineyang.png";
...
editAndSend(
  "wearing a designer evening gown",
  "#fashion",
  "auto",
  "Fine, here's your pic. Don't get used to it."
);

// Direct mode (auto-detected from "cafe")
editAndSend(
  "a luxury rooftop bar at sunset",
  "#lifestyle",
  "auto"
);
...
1. **Mirror mode context examples** (outfit focus):
   ...
   - "in high fashion streetwear"

2. **Direct mode context examples** (location/portrait focus):
   - "a luxury rooftop restaurant"
   - "a private yacht at sunset"
